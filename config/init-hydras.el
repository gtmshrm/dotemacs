...
(defhydra /hydras/buffers (:hint nil :exit t)
  "
   buffers:   _b_ → buffers          _k_ → kill buffer             _d_ → kill close
              _m_ → goto messages    _e_ → erase buffer            ^ ^
              _s_ → goto scratch     _E_ → erase buffer (force)    _f_ → reveal in os
"
...
(defhydra /hydras/files (:hint nil :exit t)
  "
   files:    _f_ → find files   _s_ → save   _D_ → delete    _y_ → copy filename   _E_ → edit as root
             _r_ → recent files     _R_ → rename    _c_ → copy file     _C_ → convert    _z_ → fzf
"
